...
- `要求建立資料夾放程式`，這是避免資訊混在一起，也方便未來的部署
...
- `實際打開檔案`驗收時，會發現 AI 有時根本沒有把任務完成
- 報表呈現的資訊`是否正確`，需要專家才有辦法確認，平常沒經手過的人是沒有這個敏銳度的

> **請 AI 修正時，他可能騙你**
> Excel 格式壞掉的問題，時常有 AI 修復後還是錯誤的狀況。

#### STEP 6：如果還是無法開啟，換個詢問方式

![Excel 想要打開時，出現了錯誤訊息](./assets/monthly-report-step6.png)
...
- 先不描述技術細節（我也不知道根因），只要求它`扮演專業工程師、重新確認與驗證，並附上錯誤檔案`
...
5. 親手驗收 — 實際打開網頁、下載 Excel／Word，`一打開就抓到 AI 自己沒發現的問題`
...
- **修 Bug 靠方法，不靠運氣**：描述不出技術細節沒關係，`換個問法、附上證據、調高 Model 與 Effort`
- **專業是最後一哩路**：你對資料的熟悉度（「管制圖裡應該算得出來」），正是把工具從 80 分推到 100 分的關鍵

> **你不需要會寫程式，但一定要驗收**
> 整個過程我們`沒有寫一行程式`；但每個「繼續」的判斷，都是自己決定的。
...
> **這個情境比月報表難很多**
> 它是一個`有很多規則要同時滿足`的問題。示範的重點`不是一鍵排出完美班表`，而是「怎麼跟 AI 討論、並設計出實用的版本」。
...
> **這讓我回想起當兵時的哨表**
> 白班夜班的哨點不同、資深資淺能站的崗位不同、每班哨最短間隔時間、如何安排休假讓重要崗位都有人，在人力不足時的狀態下如何盡可能公平。
...
- 排班結果有幾千個格子，難以逐格人工驗收，所以`要求 AI 自帶測試迴圈`
...
- 用問句而不是命令，讓 AI 彭估「公式與樣式格都能保留」才動工，`避免重寫到一半才發現不可行`
...
- 工具交給第一線試用後，`真實使用者才能告訴你專案隱藏的的地雷`
- 護理長一眼就抓出`N 接 E`的違規班型，也點出我`沒預設包班人員`、漏了`每月最多 2 種班別`的限制

![護理長試排後回報：出現 N 接 E 的違規班型，包班人員與「每月最多兩種班別」的限制也還沒被處理](assets/feedback-rule-violations.jpg)

- 並指出`跨月銜接的盲點`：上傳 7 月班表排 8 月，系統沒發現 `7/31 的 N 接 8/1 的 D`
- 根因是系統是照範例檔 `6/10～7/31` 的跨月格式去分析，而護理長`只上傳了單月`，系統設計時沒考量到相容性問題

![護理長回報系統沒抓到「月底接月初」的違規：7/31 的 N 直接接了 8/1 的 D](assets/feedback-cross-month-gap.jpg)

![對焦後找到原因：範例檔是 6/10～7/31 的跨月格式，我回覆會讓系統兼容單月份上傳](assets/feedback-cross-month-reply.jpg)

- 最珍貴的是`藏在腦袋裡的規則`：比如未包班的人以 `D+E 或 D+N` 為主、E+N 全月總數要`小於 14 天`（只差一天就能包班，`錢差很多`）
- 這些規則`不在文件中`，只有排班的第一線才知道

![依 8 月預假試排出現 7 項規則未滿足，護理長同時補充了文件裡沒有的包班潛規則](assets/feedback-hidden-rules.jpg)

> **真實的使用者會發現問題**
> 工具做完不是結束，`真實使用者試用過`，它才會慢慢長成「能用」的樣子。
...
> **工程師陌生的名詞，正是你的日常**
> 「包班」「例假」「階」「大夜接白班」，這些工程師是沒有概念的，卻是`第一線人員最熟悉的事`。
...
6. 給真實使用者試 — 護理長試排抓出`跨月盲點與包班潛規則`——優化是沒有盡頭的路
[/flow]
