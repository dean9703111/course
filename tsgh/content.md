...
![輸出排班：違規會以紅框標示，並給予支援建議](./assets/schedule-work-gen5-1.png)

![輸出排班：確認通過所有規則驗證](./assets/schedule-work-gen5.png)
...
> **透過白話文跟 AI 對話、邊聊邊生成程式的做法**
> - **優點**：可以快速完成初版
> - **缺點**：結果較不可控、未來較難維護
> 若沒有程式背景，重點在於`把想做的事講清楚，讓 AI 動手`
...
> **工程師的角色，也在改變**
> 當「把功能寫出來」越來越容易，工程師真正的價值，就從「會不會寫程式」移到`懂不懂這個產業、知不知道為什麼要做`。大家在臨床累積的 know-how，就屬於這類`最難被取代的價值`。
...
[flow]
- Cowork 有能力新增、修改、刪除你電腦上的檔案 - 想想如果做到一半`額度用完`會怎樣
...
- 改壞不是最可怕的 — 最可怕的是`你不知道它被改壞了`；其實人寫程式也會有一樣的問題，只是 AI `速度太快`，把問題加倍放大
