...
![雙擊資料夾內的 index.html 開啟網頁](./assets/monthly-report.png)

![上傳月報需要參考的檔案](./assets/monthly-report2.png)

![生成月報](./assets/monthly-report3.png)
...
![雙擊資料夾內的 index.html 開啟網頁](./assets/work -schedule.png)

![上傳上個月的排班](./assets/work -schedule2.png)

![輸出排班確認符合預期](./assets/work -schedule3.png)
...
### 🪄 Step 1：先讓它讀懂規則，列出來給我確認

```prompt [label="先摸清規則，不要急著動手"]
這個資料夾裡有月報表 Excel 和來源報表。先不要修改任何檔案。
請你讀完後，把以下規則整理成清單列給我確認：
1. 月報的「本院分子／分母／數值」各要用 VLOOKUP 比對哪幾份來源報表的哪個欄位
2. 燈號欄位的判斷邏輯（正向／負向指標、閾值、目標值）
3. SPC 圖要標註 >2σ／>3σ 的判斷方式
4. 依燈號填色的規則（紅燈整列紅、黃燈整列黃）
我確認後，我們再一項一項動手。
```

### ⚙️ Step 2：一項一項比對、判燈號、標註

```prompt [label="執行，但守住三條防線"]
規則沒問題。請依剛剛的清單處理當月數值：
1. 逐指標用 VLOOKUP 比對來源報表，把分子／分母／數值填入，每個數值標出處（來自哪份檔案）
2. 依閾值公式判斷燈號，並依燈號整列填色
3. SPC 欄位標註 >2σ／>3σ
最重要的一條：**看不清楚、辨識不確定、或來源找不到的格子，先留空並標黃，絕對不要自己猜數字。**
```

### 📝 Step 3：產出 WORD 摘要表

```prompt [label="彙整成摘要表"]
根據剛剛判好的燈號，產生 WORD 摘要表：
把紅燈指標、黃燈指標分別依「構面」分類填入對應表格。
每一筆都標明來源，方便我逐筆核對。
```

### ✅ 怎麼驗證（這步不能省）
- 拿到成品後，**特別看金額、日期、數值有沒有讀錯**
- 確認辨識不準的格子是「留空標黃」，而不是被 AI 自己填了一個看似合理的數字
- 每個結論都能追到來源檔案——不能追溯的，就不能信
...
### 🔁 用「逐步引導」逼近答案
[flow]
1. 讀現況 — 讓 Cowork 讀懂現行班表與班別代碼（D／E／N、例／休／特／生理…）
2. 給初版 — 讓它先產出第一版排班（這只是起點，一定不完美）
3. 丟約束 — 把硬約束一條一條餵進去，當作驗證條件
4. 自查違規 — 要它逐條檢查，違反的格子標黃、列出「哪一格違反哪一條」
5. 修正再驗 — 針對違規調整，再跑一次檢查
[/flow]

```prompt [label="關鍵：要它自己抓違規，不准假裝滿足"]
這是第一版排班。請針對以下每一條硬約束逐一檢查，
用表格列出：哪些格子違反了哪一條規則，並把那些格子標黃。
不要為了讓表格看起來完整，就假裝所有規則都滿足了。
沒把握的地方，明確告訴我「這裡我不確定」。
```
