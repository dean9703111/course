...
要嘛`沿用長對話`（上下文已混亂），要嘛`開新對話`（又得重講專案背景）
2. 手動餵上下文 — 複製`錯誤訊息`、`stack trace`、片段程式碼；大檔或整個 repo 無法直接「打開」，常要刪減或分多則訊息貼上
3. 下指令取得建議 — 模型回`步驟說明`或`程式碼區塊`，但不知道你本機路徑、分支、依賴版本是否一致
4. 回到本機手動套用 — 在 IDE 貼上、改檔、存檔；無法由 AI `直接改你的檔案`或`接續上一個指令的退出碼`
5. 跑指令或跑服務驗證 — 失敗再回網頁貼新錯誤；多輪往返時，對話越來越長，`容易遺漏前提`或`重複已試過的方向`
6. 需要證據時再補貼 — 例如要它幫忙看測試輸出，你得自己跑 `npm test` 再把 log 貼回去；`無法自動觸發測試 / Lint / CI`
7. 收尾靠人 — 合併分支、開 PR、對照規格是否做完，網頁 AI通常`不參與你的 Git 狀態`，也不會幫你記「這個 repo 的慣例」
[/flow]

#### 因此會有如下痛點

- **往返成本高**：每一輪都要人當「搬運工」，錯誤訊息與程式狀態很難一次貼完整
- **上下文失真**：對話一長就容易忘前提，或新開對話要重講，和「整個專案的真實狀態」有落差
- **無法閉環驗證**：測試、Lint、建置、CI 都要自己跑；AI 給的程式碼是否真能過關，取決於你手動驗證到多細
- **與協作流程脫節**：分支、PR、Code Review、部署通常不在網頁對話裡，容易變成「聊天室裡很順、進 repo 才發現對不起來」
...
> 工具間的差異沒有你想像的大，MCP、Rules、Commands、Skills，其實主流工具都有支援
