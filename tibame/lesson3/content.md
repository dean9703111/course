...
> **在 GitHub 會由 GitHub Actions 完成**
> 在 Repository 根目錄 `.github/workflows/` 放一份 YAML 設定檔，描述「`什麼事件發生`時，要`依序做哪些事`」，GitHub 就會幫你在雲端的乾淨機器上執行。
...
- 上線流程靠記憶力，**漏一個步驟**，半夜被叫起來救火

CI/CD 的價值不是「做得比人好」，而是**它不會忘記、不會偷懶、不會看心情**，每一次 push 都用`同一套標準檢查`。
...
- 還記得前面建好的 `CI/CD` 嗎？只要測試不過，**合併按鈕直接變灰色**，不只是「亮紅燈提醒你」
...
> **小提醒**
> 如果前面的操作`涵蓋讓 AI 解決合併回 develop 的衝突(feature/upgrade-major-dependencies)`。
...
| 自己租 VM | 用 Zeabur |
| --- | --- |
| 要自己裝環境、設防火牆、上 SSL | 平台`自動處理`建置、HTTPS、網域 |
