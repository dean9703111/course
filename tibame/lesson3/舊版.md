...
1. 點擊左上角頭像 → 設定 → API 金鑰
2. 產生新的 API 金鑰（可選）
3. 點擊複製 icon 來保存 API 金鑰

![複製金鑰](./assets/zeabur-api-key.png)
...
[lab-session title="🛠️  實作練習"]
- 安裝 Zeabur 官方 MCP
- 在 Claude Code 確認 Zeabur MCP 啟用
[/lab-session]

### 🚀 用白話文完成部署

Zeabur 有兩種讓 AI 操作的方案，一個是 MCP，另一個是 Skill。

根據個人使用經驗，`MCP 穩定性遠遠高於 Skill`；Skill 鬼打牆、搞錯帳號、登入失敗等問題極多。

```prompt [label="部署到 Zeabur"]
使用 Zeabur MCP，參考專案 docker-compose-prod.yml 的架構，部署到伺服器 [伺服器 ID]
然後使用「vms-dean」作為子域名，若重名就在後面加上 6 位 hash 值
```

> **使用 MCP 要注意的事情**
> Zeabur MCP 將的`建立服務、設定、查 log`都變成 AI 能呼叫的工具，你描述`想達成的結果`，AI 負責`一步步呼叫 API`，不用在到後台來回點。
