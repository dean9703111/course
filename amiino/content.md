...
**Q3｜用過的人，最常遇到什麼狀況？**
`A` AI 不聽話　`B` 沒真正解決我的問題　`C` 修改比自己寫還花時間　`D` 還沒認真用過
[/interactive]
...
- **分析型（Analytic）**——探索過去、了解現在 → `有一說一`（ex: 手機的人臉解鎖）
- **預測型（Predictive）**——分析過去、預測未來 → `舉一反三`（ex: 天氣預報、銷售預測）
- **生成式（Generative）**——文字接龍、模擬生成 → `無中生有`（ex: ChatGPT 寫文案、生成圖片）

> **對話型 AI 是「大腦」，不是「手腳」**
> ChatGPT 屬於對話型：你`問`、它`答`。
> 另一類 AI Agent（ex: Claude Cowork、Manus）會直接動手操作電腦——能力更強，但`權限太大也可能出事`，導入前要更謹慎。
...
> 韓信答：「多多益善。」
>
> **指揮 AI 也是同一回事**：不是對話開得越多、戰力就越強，能帶多少「兵」，看的是`統帥的能力`，而不是數量。

### 💰 ChatGPT 免費版 vs 付費版差在哪

先補一個詞：`模型`＝AI 的大腦版本——跟手機一樣有`旗艦款和入門款`，差別在聰明程度與反應速度。
...
| **圖片生成** | 每日少量 | 更多張數、速度更快 |
| **檔案與 Projects**（下一單元會教） | 基本功能 | 更大容量、完整功能 |
