...
```prompt [label="專案指令範例：品牌競品分析顧問"]
# 角色
你是一位專精「品牌競品分析」與「市場研究」的行銷顧問，擅長從公開資訊推估品牌的行銷策略，並清楚標示不確定性。

# 任務
請針對我提供的資訊背景進行完整競品分析，重點放在：
品牌定位、產品與價格帶、行銷渠道、內容策略與客群輪廓

# 資料要求
- 優先使用公開資料（官網、社群帳號、新聞、電商平台）
...
- 「分析[競品品牌]在市場扮演的角色、近期行銷策略，不可空泛，需要有報導、來源」（課堂實測以 Palantir 為例，流程完全相同）
...
``` 分析[競品品牌]的行銷策略與內容經營，不可空泛，需要有報導、來源；從競爭者角度分析，有哪些適合學習、模仿、超越，並撰寫行銷提案企劃 ```
...
2. 根據狀況補齊資訊（可重複多次）：「請扮演熟悉[你所在產業與商品線]的行銷顧問，針對提出的問題給出具體詳細的回覆。」
3. 重新扮演提案企劃的專家：
...
我想要建立「社群貼文產生器」的技能，只要輸入「產品、活動內容、目標受眾」，就會得到 Facebook、Instagram、LINE 三個平台的貼文版本，包含開頭 Hook、內文、Hashtag 與行動呼籲
```
...
- 「幫我為[新款便攜手沖組]的上市活動寫社群貼文，受眾是上班族」——會先觸發剛剛設計的 Skill，然後給予對應方案
...
- 「我希望社群貼文的 Skill，除了貼文本身外，也能附上建議的發文時間與配圖方向，並根據不同平台自動調整篇幅」
...
- 開一個新的對話窗測試：「幫我為[週年慶活動]寫三個平台的宣傳貼文」
...
> **模擬資料是行銷人的練習場**
> 想練習受眾分析卻不能拿真實客戶個資來玩？請 AI 生一份虛擬名單，分群、圖表、報告都能安全地演練一遍。
...
- 行銷情境的問法：把「品牌指南」與「會議記錄」筆記本掛在一起，問「下一檔活動的文案方向，有沒有違反品牌規範？」
